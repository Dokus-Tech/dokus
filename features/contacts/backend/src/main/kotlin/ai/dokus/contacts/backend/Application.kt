package ai.dokus.contacts.backend

import ai.dokus.contacts.backend.config.configureDependencyInjection
import ai.dokus.contacts.backend.plugins.*
...
private val logger = LoggerFactory.getLogger("ContactsApplication")
...
fun main() {
    ...
    Runtime.getRuntime().addShutdownHook(Thread {
        logger.info("Shutting down Contacts Service gracefully...")
...
fun Application.module(appConfig: AppBaseConfig) {
    logger.info("Starting Dokus Contacts Service...")
    ...
    logger.info("Dokus Contacts Service started successfully on port ${appConfig.ktor.deployment.port}")
}
