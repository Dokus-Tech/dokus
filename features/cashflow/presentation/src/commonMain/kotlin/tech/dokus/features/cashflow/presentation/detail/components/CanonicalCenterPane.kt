...
import tech.dokus.features.cashflow.presentation.detail.components.bankstatement.CanonicalBankStatementView
import tech.dokus.features.cashflow.presentation.detail.components.comparison.DocumentComparisonPane
...
internal fun CanonicalCenterPane(
    ...
    state.sourceViewerState?.let { viewerState ->
        SourceViewerCenter(
            ...
            onToggleTechnicalDetails = { onIntent(DocumentDetailIntent.Preview(DocumentPreviewIntent.ToggleSourceTechnicalDetails)) },
            onRetry = { onIntent(DocumentDetailIntent.Preview(DocumentPreviewIntent.OpenSourceModal(viewerState.sourceId))) },
            modifier = modifier,
        )
        return
    }
    ...
    if (uiData is DocumentUiData.BankStatement) {
        CanonicalBankStatementView(
            ...
            onToggleTransaction = { index ->
                onIntent(DocumentDetailIntent.ToggleBankStatementTransaction(index))
            },
            onReject = { onIntent(DocumentDetailIntent.ShowRejectDialog) },
            onConfirm = { onIntent(DocumentDetailIntent.Confirm) },
...
private fun SourceViewerCenter(
    contentState: DocumentDetailState,
