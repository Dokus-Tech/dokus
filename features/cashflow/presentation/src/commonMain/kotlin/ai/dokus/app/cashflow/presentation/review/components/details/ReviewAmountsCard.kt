            ...
            when (state.editableData.documentType) {
                DocumentType.Invoice -> {
                    val fields = state.editableData.invoice
                        ?: ai.dokus.app.cashflow.presentation.review.EditableInvoiceFields()
                    PTextFieldStandard(
                ...
                DocumentType.Bill -> {
                    val fields = state.editableData.bill
                        ?: ai.dokus.app.cashflow.presentation.review.EditableBillFields()
                    PTextFieldStandard(
                        fieldName = stringResource(Res.string.invoice_total_amount),
                        value = fields.amount,
                        onValueChange = {
                ...
                DocumentType.Expense -> {
                    val fields = state.editableData.expense
                        ?: ai.dokus.app.cashflow.presentation.review.EditableExpenseFields()
                    PTextFieldStandard(
                        fieldName = stringResource(Res.string.invoice_total_amount),
                        value = fields.amount,
                        onValueChange = {
