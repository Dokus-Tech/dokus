...
class DocumentDetailRouteMappingsTest {

    @Test
    fun `route with no params maps to Recent context`() {
        val route = CashFlowDestination.DocumentDetail(documentId = "doc-1")

        assertIs<CashFlowDestination.DocumentDetailQueueContext.Recent>(route.queueSource)
    }

    @Test
    fun `route with filter maps to DocumentList context`() {
        val route = CashFlowDestination.DocumentDetail(
            documentId = "doc-1",
            filter = DocumentListFilter.NeedsAttention.name,
        )

        val source = assertIs<CashFlowDestination.DocumentDetailQueueContext.DocumentList>(route.queueSource)
    ...
    fun `route with contactId maps to Contact context`() {
        ...
        val source = assertIs<CashFlowDestination.DocumentDetailQueueContext.Contact>(route.queueSource)
    ...
    fun `route with query maps to Search context`() {
        ...
        val source = assertIs<CashFlowDestination.DocumentDetailQueueContext.Search>(route.queueSource)
