...
internal inline fun <reified Input> AIAgentSubgraphBuilderBase<*, *>.documentPreparationSubGraph(
...
): AIAgentSubgraphDelegate<Input, ClassifyDocumentInput> where Input : InputWithDocumentId, Input : InputWithTenantContext, Input : InputWithUserFeedback {
    return subgraph(name = "document-preparation") {
        ...
        val injectUserFeedback by userFeedbackInjectorNode<Input>()
        ...
        nodeStart then injectTenant then injectImages then injectUserFeedback then prepareClassifyInput then nodeFinish
    }
}
