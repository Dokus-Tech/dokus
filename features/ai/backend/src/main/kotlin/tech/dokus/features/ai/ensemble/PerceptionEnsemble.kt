...
class PerceptionEnsemble<T : Any>(
    ...
    suspend fun extract(images: List<DocumentImage>): EnsembleResult<T> {
        return if (runParallel) {
            extractParallel(images)
        } else {
            extractSequential(images)
        }
    }
     ...
     */
    private suspend fun extractSequential(images: List<DocumentImage>): EnsembleResult<T> {
        logger.info("Ensemble: sequential mode (fast → expert fallback)")
     ...
     */
    private suspend fun extractParallel(images: List<DocumentImage>): EnsembleResult<T> =
        coroutineScope {
            logger.info("Ensemble: parallel mode (fast || expert)")
            val startTime = System.currentTimeMillis()
    ...
    companion object {
        ...
        ): PerceptionEnsemble<T> = PerceptionEnsemble(fastAgent, expertAgent, runParallel)
    }
}
