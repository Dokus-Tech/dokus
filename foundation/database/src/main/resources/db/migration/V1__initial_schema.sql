...
CREATE TABLE tenants (
    ...
    language VARCHAR(50) DEFAULT 'EN' NOT NULL CHECK (language IN ('EN', 'FR', 'NL', 'DE', 'ES', 'IT')),
...
CREATE TABLE clients (
    ...
    peppol_enabled BOOLEAN DEFAULT false NOT NULL,
    tags VARCHAR(500),
    notes TEXT,
    is_active BOOLEAN DEFAULT true NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TABLE invoices (
    ...
    status VARCHAR(50) NOT NULL CHECK (status IN ('DRAFT', 'SENT', 'VIEWED', 'PAID', 'PARTIALLY_PAID', 'OVERDUE', 'CANCELLED', 'REFUNDED')),
    peppol_id VARCHAR(255),
    peppol_sent_at TIMESTAMP,
    peppol_status VARCHAR(50) CHECK (peppol_status IS NULL OR peppol_status IN ('PENDING', 'SENT', 'DELIVERED', 'FAILED', 'REJECTED')),
    payment_link VARCHAR(500),
    payment_link_expires_at TIMESTAMP,
    paid_at TIMESTAMP,
    payment_method VARCHAR(50) CHECK (payment_method IS NULL OR payment_method IN ('BANK_TRANSFER', 'CREDIT_CARD', 'DEBIT_CARD', 'PAYPAL', 'STRIPE', 'CASH', 'CHECK', 'OTHER')),
    currency VARCHAR(50) DEFAULT 'EUR' NOT NULL CHECK (currency IN ('EUR', 'USD', 'GBP', 'CHF', 'CAD', 'AUD')),
    notes TEXT,
...
CREATE TABLE expenses (
    ...
    category VARCHAR(100) NOT NULL CHECK (category IN ('OFFICE_SUPPLIES', 'TRAVEL', 'MEALS', 'SOFTWARE', 'HARDWARE', 'UTILITIES', 'RENT', 'INSURANCE', 'MARKETING', 'PROFESSIONAL_SERVICES', 'TELECOMMUNICATIONS', 'VEHICLE', 'OTHER')),
    description TEXT,
...
CREATE TABLE payments (
    ...
    payment_method VARCHAR(50) NOT NULL CHECK (payment_method IN ('BANK_TRANSFER', 'CREDIT_CARD', 'DEBIT_CARD', 'PAYPAL', 'STRIPE', 'CASH', 'CHECK', 'OTHER')),
...
CREATE TABLE bank_connections (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    provider VARCHAR(50) NOT NULL CHECK (provider IN ('PLAID', 'YODLEE', 'TINK', 'SALT_EDGE', 'MANUAL')),
    institution_id VARCHAR(100) NOT NULL,
    institution_name VARCHAR(255) NOT NULL,
    account_id VARCHAR(255) NOT NULL,
    account_name VARCHAR(255),
    account_type VARCHAR(50) CHECK (account_type IS NULL OR account_type IN ('CHECKING', 'SAVINGS', 'CREDIT_CARD', 'BUSINESS', 'INVESTMENT')),
    currency VARCHAR(50) DEFAULT 'EUR' NOT NULL CHECK (currency IN ('EUR', 'USD', 'GBP', 'CHF', 'CAD', 'AUD')),
...
CREATE TABLE vat_returns (
    ...
    status VARCHAR(50) NOT NULL CHECK (status IN ('DRAFT', 'SUBMITTED', 'ACCEPTED', 'REJECTED', 'PAID')),
    filed_at TIMESTAMP,
    paid_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
...
CREATE TABLE audit_logs (
    ...
    entity_type VARCHAR(50) NOT NULL CHECK (entity_type IN ('INVOICE', 'INVOICE_ITEM', 'CLIENT', 'EXPENSE', 'PAYMENT', 'BANK_CONNECTION', 'BANK_TRANSACTION', 'VAT_RETURN', 'USER', 'TENANT', 'SETTINGS', 'ATTACHMENT')),
