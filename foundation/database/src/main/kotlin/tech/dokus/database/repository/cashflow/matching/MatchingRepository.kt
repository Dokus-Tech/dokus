    ...
    suspend fun loadCandidateEntries(
        ...
        CashflowEntriesTable.selectAll().where { condition }.map { row ->
            CashflowEntryEntity(
                ...
                amountGross = Money.fromDbDecimal(row[CashflowEntriesTable.amountGross], row[CashflowEntriesTable.currency]),
                amountVat = Money.fromDbDecimal(row[CashflowEntriesTable.amountVat], row[CashflowEntriesTable.currency]),
                remainingAmount = Money.fromDbDecimal(row[CashflowEntriesTable.remainingAmount], row[CashflowEntriesTable.currency]),
